...
M.run = function()
    -- If buffer is modified, write it.
    ...
    client.request("workspace/executeCommand", lens.command, function(...)
        local result = vim.lsp.handlers['workspace/executeCommmand'](...)
...
M.refresh_virtlines = function()
    ...
    vim.lsp.buf_request_all(bufnr, "textDocument/codeLens", params, function(err, result, _,_)
        if err then
            print("[LSP]: Does not support code lens")
            return
        end
        ...
        vim.api.nvim_buf_clear_namespace(bufnr, ns, 0, -1)
         if not virtlines_enabled then
             print("[LSP]: Virtual lines are not enabled")
             return
         end
